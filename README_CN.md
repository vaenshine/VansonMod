...
**VansonMod** 是一款专为 **TrollStore (巨魔)** 用户打造的独立内存修改神器。

它打破了传统修改器需要越狱的限制，让你在 **iOS 14.0 - 17.0** 的非越狱设备上，也能像使用 CE (Cheat Engine) 一样扫描内存数值，甚至直接修改 App 的汇编指令 (RVA Patch)。它利用巨魔的高级权限运行，实现了对其他进程的“降维打击”。

### 📱 兼容性说明
*   **系统版本：** 完美支持 **iOS 14.0 - 17.0**。
*   **核心要求：** 设备必须安装 **TrollStore (巨魔商店)**。
*   **设备支持：** 支持所有 64 位 (arm64 / arm64e) iPhone 和 iPad。
